...
# -*- coding: utf-8 -*-
"""
Created on Mon May 19 19:59:40 2025
...
def minVarianceStrategy(x):
    ...
    c_inv = np.linalg.inv(c)     # 协方差矩阵的逆
    ones = np.ones(len(c_inv))   # 全为1的向量
...
def evaluateMinVarianceStrategy(M):
    """评估最小方差策略的表现
    Evaluate the performance of minimum variance strategy"""
    ...
    print('min sharp ratio: insample = %4.4f,outpfsample=%4.4f' % (SRI,SRO))
    print('min CEQ:outofsample=%4.6f'%CEQ)
    print('min turnover:outofsample=%4.4f'%TO)
    return SRI

M=60
print('Window size = %d' %M)
SRI= evaluateMinVarianceStrategy(M)
